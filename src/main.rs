...
async fn main() -> std::io::Result<()> {
    ...
    let api_key = env::var("API_KEY").unwrap_or_else(|| "default-api-key".to_string());
    let port = env::var("PORT").unwrap_or_else(|| "8080".to_string()).parse::<u16>().expect("Invalid PORT value");
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/health", web::get().to(health_check)) // Health check route added
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
