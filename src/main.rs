use actix_web::{web, App, HttpServer, Responder, HttpResponse, HttpRequest}; use std::sync::{Arc, Mutex}; use tokio::sync::Mutex as TokioMutex; use tokio::time::{interval, Duration}; use std::fs; use chrono::Utc; use std::path::Path; use rusqlite::Connection; use std::env; use dotenvy::dotenv;
...
// Backup Database fn backup_database(db: &Connection) { let backup_dir = "./backups/"; if !Path::new(backup_dir).exists() { fs::create_dir_all(backup_dir).expect("Failed to create backup directory"); } let timestamp = Utc::now().format("%Y-%m-%d_%H-%M-%S").to_string(); let backup_path = format!("{}/backup_{}.db", backup_dir, timestamp); match fs::copy("./earn_vault.db", &backup_path) { Ok(_) => println!("Database backed up successfully at {}", backup_path), Err(e) => eprintln!("Database backup failed: {}", e), } }
